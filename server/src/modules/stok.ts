...
async function currentQty(db: ReturnType<typeof getDb>, tenantId: string, depoId: string, balikCinsId: string): Promise<number> {
  const [row] = await db.select({ q: sql<number>`round(coalesce(sum(${s.stokHareketler.giris} - ${s.stokHareketler.cikis}),0),2)` })
...
export async function registerStok(app: FastifyInstance) {
  ...
  app.get('/api/stok/bakiye', async (req) => {
    ...
    return db.select({
      ...
      bakiye: sql<number>`round(sum(${s.stokHareketler.giris} - ${s.stokHareketler.cikis}),2)`,
  ...
  app.get('/api/stok/ekstre', async (req) => {
    ...
    let bakiye = 0;
    return hareketler.map((h) => { bakiye = round2(bakiye + h.giris - h.cikis); return { ...h, bakiye }; });
  });
  ...
  app.post('/api/stok/transfer', async (req, reply) => {
    ...
    const db = getDb(); const tenantId = req.ctx.tenantId; const belgeId = nanoid();
    transaction(() => {
      postStok(db, tenantId, { depoId: p.data.kaynakDepoId, balikCinsId: p.data.balikCinsId, tarih: p.data.tarih, cikis: p.data.miktar, belgeTip: 'TRANSFER', belgeId });
      postStok(db, tenantId, { depoId: p.data.hedefDepoId, balikCinsId: p.data.balikCinsId, tarih: p.data.tarih, giris: p.data.miktar, belgeTip: 'TRANSFER', belgeId });
    });
    await writeAudit(db, req.ctx, 'stok_transfer', belgeId, 'post', null, p.data);
    reply.code(201); return { id: belgeId };
  });
